...
source "$script_dir/q_sys_mem_if_ddr3_emif_0_p0_report_timing_core.tcl"
...
foreach inst $instances {
	...
	# DQS_phase offset
	set dqs_phase [ q_sys_mem_if_ddr3_emif_0_p0_get_dqs_phase $dqs_pins ]

	# Get the interface type (HPAD or VPAD)
	set interface_type [q_sys_mem_if_ddr3_emif_0_p0_get_io_interface_type $all_dq_pins]
	...
	set io_std [q_sys_mem_if_ddr3_emif_0_p0_get_io_standard [lindex $dqs_pins 0]]
	...
	if {[llength $instances] <= 1} {
		set fbasename "${::GLOBAL_q_sys_mem_if_ddr3_emif_0_p0_corename}"
	} else {
		set fbasename "${::GLOBAL_q_sys_mem_if_ddr3_emif_0_p0_corename}_${inst_id}"
	}
	...
	foreach opc $opcs {
		...
		set model_corner [q_sys_mem_if_ddr3_emif_0_p0_get_model_corner]
		...
		if {!$in_gui && [array exists assumptions_cache] &&  [info exists assumptions_cache(${::GLOBAL_q_sys_mem_if_ddr3_emif_0_p0_corename}-$inst)] } {
			set assumptions_valid $assumptions_cache(${::GLOBAL_q_sys_mem_if_ddr3_emif_0_p0_corename}-$inst)
			...
			set assumptions_valid [q_sys_mem_if_ddr3_emif_0_p0_verify_flexible_timing_assumptions $inst pins $mem_if_memtype]
			set assumptions_cache(${::GLOBAL_q_sys_mem_if_ddr3_emif_0_p0_corename}-$inst) $assumptions_valid
		}	
		...
		q_sys_mem_if_ddr3_emif_0_p0_perform_flexible_write_launch_timing_analysis $opcs $opcname $inst $family scale_factors $interface_type $max_package_skew $dll_length $period pins t summary MP IP board

		#######################################
		# Read Analysis

		q_sys_mem_if_ddr3_emif_0_p0_perform_flexible_read_capture_timing_analysis $opcs $opcname $inst $family scale_factors $io_std $interface_type $max_package_skew $dqs_phase $period $all_dq_pins pins t summary MP IP board fpga
		q_sys_mem_if_ddr3_emif_0_p0_perform_resync_timing_analysis $opcs $opcname $inst ${::GLOBAL_q_sys_mem_if_ddr3_emif_0_p0_corename} $family scale_factors $io_std $interface_type $period pins t summary MP IP board fpga SSN

		#######################################
		# PHY and Address/command Analyses

		q_sys_mem_if_ddr3_emif_0_p0_perform_ac_analyses  $opcs $opcname $inst scale_factors pins t summary IP		
		q_sys_mem_if_ddr3_emif_0_p0_perform_phy_analyses $opcs $opcname $inst $inst_controller pins t summary IP


		#######################################
		# Bus Turnaround Time Analysis
		q_sys_mem_if_ddr3_emif_0_p0_perform_flexible_bus_turnaround_time_analysis $opcs $opcname $inst $family $period $dll_length $interface_type $tJITper $tJITdty $tDCD $pll_steps pins t summary MP IP SSN board ISI


		#######################################
		# Postamble analysis
		q_sys_mem_if_ddr3_emif_0_p0_perform_flexible_postamble_timing_analysis $opcs $opcname $inst scale_factors $family $period $dll_length $interface_type $tJITper $tJITdty $tDCD $DQSpathjitter pins t summary MP IP SSN board ISI
	...
	set summary [lsort -command q_sys_mem_if_ddr3_emif_0_p0_sort_proc $summary]

	set f -1
	if { [q_sys_mem_if_ddr3_emif_0_p0_get_operating_conditions_number] == 0 } {
		set f [open $fname w]

		puts $f "Core: ${::GLOBAL_q_sys_mem_if_ddr3_emif_0_p0_corename} - Instance: $inst"
	...
	post_message -type info "Core: ${::GLOBAL_q_sys_mem_if_ddr3_emif_0_p0_corename} - Instance: $inst"
	...
	foreach summary_line $summary {
		...
		if {$su != "--"} {
			set su [ q_sys_mem_if_ddr3_emif_0_p0_round_3dp $su]
		}
		if {$hold != "--"} {
			set hold [ q_sys_mem_if_ddr3_emif_0_p0_round_3dp $hold]
		}
	...
	post_message -type critical_warning "Timing analysis was performed on core ${::GLOBAL_q_sys_mem_if_ddr3_emif_0_p0_corename} using Quartus II v12.1 SP1 with a preliminary timing model and constraints. You must regenerate this IP in a future version of Quartus II to update the timing constraints to match the timing model."
	incr inst_id
}
